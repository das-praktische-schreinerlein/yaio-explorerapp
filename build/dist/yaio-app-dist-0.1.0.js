...
Yaio.NodeEditor = function(appBase) {
    ...
    me.setAutoGenerateNodeDesc = function(elementId) {
        ...
        element.val('<!---DoAutoGenerateDesc SHOWCONTENT--->\n').trigger('select').triggerHandler('change');
        me.appBase.DataUtils.callUpdateTriggerForElement(element);
    };
...
Yaio.Layout = function(appBase) {
     ...
     */
    me.addSetAutoGenerateNodeDescToElements = function (filter) {
        ...
        me.$(filter).append(function (idx) {
            ...
            if (me.$(label).attr('setAutoGenerateNodeDescAdded')) {
                console.error('addSetAutoGenerateNodeDescToElements: SKIP because already added: ' + me.$(label).attr('for'));
                return link;
            }
            ...
            if (forElement.length > 0) {
                ...
                var clickHandler = ymfAppBaseVarName + '.YaioNodeEditor.setAutoGenerateNodeDesc(\'' + forElement.attr('id') + '\'); return false;';
                link += '<a href="#" id="setAutoGenerateNodeDesc4' + forElement.attr('id') + '"' +
                    ' onClick="' + clickHandler + '" lang="tech"' +
                    ' data-tooltip="tooltip.command.SetAutoGenerateNodeDesc" class="button">common.command.SetAutoGenerateNodeDesc</a>';

                // set flag
                me.$(label).attr('setAutoGenerateNodeDescAdded', 'true');
                console.log('addSetAutoGenerateNodeDescToElements: add : ' + forName + ' for ' + forElement.attr('id'));
            }
            return link;
        });
    };
...
yaioApp.controller('NodeShowCtrl', function($rootScope, $scope, $location, $routeParams,
    ...
    $scope.loadCurrentNodeIdSuccessHandler = function(nodeId, options, yaioNodeActionResponse) {
        ...
        if (state === 'OK') {
            ...
            $scope.nodeHierarchy = yaioUtils.getService('YaioBase').getNodeHierarchy(yaioNodeActionResponse.node);
